...
# Add Lora settings
from peft import LoraConfig, get_peft_model  # NEW: Import LoRA modules
lora_config = LoraConfig(
    r=8,                          # Rank of the low-rank matrices
    lora_alpha=32,                # Scaling factor (alpha/r stabilizes training)
    target_modules=["q_proj", "v_proj"],  # Target attention matrices in Wav2Vec2
    lora_dropout=0.05,            # Dropout for LoRA layers
    bias="none",                  # Don't add bias parameters
    task_type="CTC",              # Specify CTC task type
    gradient_checkpointing=False  # CRITICAL: Disable PEFT's gradient checkpointing
)
...
model = Wav2Vec2ForCTC.from_pretrained(
    ...
    mask_time_prob=0.05,
    layerdrop=0.1,
    gradient_checkpointing=True,   # Keep base model's checkpointing
    ctc_loss_reduction="mean",
    pad_token_id=processor.tokenizer.pad_token_id,
    vocab_size=len(processor.tokenizer),
    torch_dtype=torch.float16,     # Load model in FP16 to save VRAM
)
