...
training_args = TrainingArguments(
    output_dir="./wav2vec2-large-xlsr-cantonese",
    group_by_length=True,
    # Increased batch size due to LoRA memory savings
    per_device_train_batch_size=8,          # Was 4, now doubled
    gradient_accumulation_steps=2,          # Effective batch size 16
    gradient_checkpointing=True,             # Combine with LoRA
    remove_unused_columns=False,
    evaluation_strategy="steps",
    eval_steps=400,
    num_train_epochs=10,
    fp16=True,                              # Essential for V100
    fp16_backend="amp",
    fp16_full_eval=True,
    logging_strategy="steps",
    logging_steps=400,
    learning_rate=1e-3,                     # Slightly higher LR for LoRA
    warmup_steps=500,
    save_steps=2376,
    save_total_limit=3,
    ...
    # New memory-saving parameters
    ddp_find_unused_parameters=False,
    tf32=False,                             # Disable TensorFloat-32
)

# ========== MODIFIED TRAINER SETUP ========== #
trainer = Trainer(
    model=model.to(device),
    data_collator=data_collator,
    args=training_args,
...
# Save only LoRA weights for compact storage
